...
<body>
    ...
    <div class="legend" id="typeLegend">
        ...
        <div class="legend-item">
            Node shape:
            <strong>Square</strong> = Pamphlet,
            <strong>Circle</strong> = Source cited/referenced in pamphlet
        </div>
    ...
    <script>
        ...
        async function loadNetwork() {
            ...
            // 1. Fetch the data
            const rawNodes = await fetch(
                "{{ '/assets/citation-network/nodes.json' | relative_url }}"
            ).then(res => res.json());

            const rawEdges = await fetch(
                "{{ '/assets/citation-network/edges.json' | relative_url }}"
            ).then(res => res.json());


            // 2. CLEAN THE DATA
            // Removes BOM and other non-standard characters from keys.
            ...
            const themes = [
                ...new Set(
                    nodesData.map(node => themeFor(node.Category))
                )
            ].sort();
            ...
            themes.forEach(theme => {
                ...
                item.innerHTML = `
                    <span
                        class="legend-icon"
                        style="background-image: url('${themeIconMap[theme]}')">
                    </span>
                    ${theme}
                `;
...

                    {
                        ...
                        style: {
                            ...
                            "background-image": function(ele) {
                                ...
                                return themeIconMap[ele.data("theme")]
                                    || themeIconMap[UNCATEGORIZED];
                            },
            ...
            cy.on("mouseover", "node", function(event) {
                ...
                tooltip.innerHTML =
                    ...
                    (data.theme && data.type !== "pamphlet"
                        ? "Theme: " + data.theme + "<br>"
                        : "") +

                    (data.author_origin
                        ? "Origin: " + data.author_origin
                        : "");
                ...
                const neighborhood =
                    connectedEdges.connectedNodes().union(node);
            ...
            cy.on("mouseover", "edge", function(event) {
                ...
                tooltip.innerHTML =
                    ...
                    (data.citation_method
                        ? "Method: " + data.citation_method + "<br>"
                        : "") +

                    (data.weight
                        ? "Number of Mentions: " + data.weight + "<br>"
                        : "") +
            ...
            cy.on("mousemove", function(event) {
                ...
                tooltip.style.left =
                    (event.originalEvent.pageX + 10) + "px";

                tooltip.style.top =
                    (event.originalEvent.pageY + 10) + "px";
            });
        }
